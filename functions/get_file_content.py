...
def get_file_content(working_directory, file_path):
    ...
    try:
        ...
        return f"Error: Could not get content {e}"
    
schema_get_file_content = types.FunctionDeclaration(
    name="get_file_content",
    description="Displays the content of a given file, constrained to the working directory",
    parameters=types.Schema(
        type=types.Type.OBJECT,
        properties={
            "file_path": types.Schema(
                type=types.Type.STRING,
                description="The file that should have it's contents displayed. The contents should be truncated at 10,000 characters"
            )
        }
    )
)